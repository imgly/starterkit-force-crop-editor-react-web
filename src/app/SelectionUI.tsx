...
interface SelectionUIProps {
  images: ImageConfig[];
  presets: CropPreset[];
  onComplete: (image: ImageConfig, preset: CropPreset, mode: CropModeId) => void;
}
...
const CROP_MODES: CropModeOption[] = [
  ...
  },
  {
    id: 'ifNeeded',
    label: 'If Needed',
    description: 'This mode opens the Crop Mode only if image does not match the aspect ratio.'
  },
  {
    id: 'silent',
    label: 'Silent',
...
export default function SelectionUI({ images, presets, onComplete }: SelectionUIProps) {
  ...
  const handleOpenEditor = () => {
    onComplete(images[selectedImageIndex], presets[selectedPresetIndex], selectedMode);
  };
    ...
    <div className={styles.selectionPage}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.modeSelector}>
          ...
          <p className={styles.modeDescription}>{selectedModeOption?.description}</p>
        </div>
      </div>

      {/* Open Editor Button */}
      <button className={styles.openButton} onClick={handleOpenEditor} data-cy="applyButton">
        Open Editor
      </button>
    </div>
  );
